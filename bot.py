...
async def admin_add_balance(msg: Message):
    ...
    await dp.current_state(user=msg.from_user.id).set_state(AdminAddBalanceState.waiting_user_id)
...
async def receive_local_number(msg: Message, state: FSMContext):
    ...
    text = (
        ...
        f"👤 User: {uid}\n"
        f"💵 Amount: ${withdrawals[request_id]['amount']:.2f}\n"
...
async def show_referral(msg: Message):
    ...
    ref_list = user.get("referrals", [])
    text = f"👥 Referral Code: {user.get('ref_code', 'N/A')}\n"
    text += f"Number of referrals: {len(ref_list)}\n"
    text += "\nReferred Users:\n"
    for r in ref_list:
        text += f"- {r}\n"
...
async def show_balance(msg: Message):
    ...
    await msg.answer(f"💰 Balance-kaaga hadda: ${user.get('balance', 0.0):.2f}")
