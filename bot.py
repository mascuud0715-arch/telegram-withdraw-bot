...
async def withdrawal_local(msg: Message, state: FSMContext):
    ...
    if balance < 1:
        await msg.answer("❌ Balance-kaaga ma gaadhin $1, ma dalban kartid.")
        return

    await msg.answer(f"🤑 Soo dir Number-kaaga si lacagta laguu diro (min $1). Current balance: ${balance:.2f}")
...
async def receive_crypto_address(msg: Message, state: FSMContext):
    ...
    if users.get(uid, {}).get("balance", 0) < 1:
        await msg.answer("❌ Balance-kaaga ma gaadhin $1, ma dalban kartid.")
        return
